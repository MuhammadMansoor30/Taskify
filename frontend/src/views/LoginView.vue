<template>
    <div class="d-flex flex-column justify-content-center align-items-center" style="height: 100vh;">
        ...
        <b-form @submit.prevent="submitForm()" class="w-50">
            <b-form-group id="email" label="Enter Your Email" label-for="email-input" label-class="text-left"
                class="fs-4">
                <div>
                    <b-form-input id="email-input" type="email" v-model="email" required placeholder="test@email.com"
                        :class="{ 'is-invalid': emailError, 'fs-5': true }" />
                    <b-form-invalid-feedback v-if="emailError">{{ emailErrorMessage }}</b-form-invalid-feedback>
                </div>
            </b-form-group>

            <b-form-group id="password" label="Enter Your Password" label-for="password-input" label-class="text-left"
                class="fs-4">
                <div class="d-flex">
                    <b-input-group class="w-100">
                        <b-form-input id="password-input" :type="passwordVisible ? 'text' : 'password'"
                            v-model="password" required placeholder="*********"
                            :class="{ 'is-invalid': passwordError, 'fs-5': true }" />
                        ...
                        <b-form-invalid-feedback v-if="passwordError">{{ passwordErrorMessage
            ...
            try {
                ...
                if (data) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        timer: 1000,
                        title: "Login In Failed Enter correct Details",
                        showConfirmButton: false,
                    });
                }
            }
